...
// Robot Gardien Avancé et Professionnel - Flottant dans la zone de contenu
const AdvancedGuardianRobot = ({ isDark }: { isDark: boolean }) => {
  ...
  // Initialize robot ID only on client side
  useEffect(() => {
    setRobotId(Math.floor(Math.random() * 9999).toString().padStart(4, "0"))
  }, [])

  useEffect(() => {
    const moveInterval = setInterval(() => {
      setPosition((prev) => {
        let newX = prev.x + direction * 0.3
  ...
  return (
    <div
      className="fixed z-30 transition-all duration-300 ease-in-out pointer-events-none"
      style={{
        left: `${position.x}%`,
        top: `${position.y}%`,
    ...
    >
      <div className="relative group">
        ...
        <div
          className={`relative w-16 h-20 bg-gradient-to-b ${getStatusColor()} rounded-2xl shadow-2xl border-2 ${
            isDark ? "border-gray-600" : "border-white/30"
          }`}
        >
          {/* Tête avec écran */}
          <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
            <div className="w-12 h-8 bg-gradient-to-b from-gray-800 to-gray-900 rounded-t-xl border-2 border-gray-600">
              {/* Écran avec données */}
              <div className="w-full h-full bg-black rounded-t-lg p-1 flex flex-col justify-center items-center">
                <div className="text-green-400 text-[6px] font-mono leading-tight">
          ...
          <div className="absolute -top-6 left-2">
            <div className="w-0.5 h-3 bg-gray-400 rounded-full">
              <div className="w-2 h-2 bg-red-500 rounded-full -ml-0.5 animate-pulse"></div>
            </div>
          </div>
          <div className="absolute -top-6 right-2">
            <div className="w-0.5 h-3 bg-gray-400 rounded-full">
              <div className="w-2 h-2 bg-blue-500 rounded-full -ml-0.5 animate-pulse"></div>
            </div>
          </div>

          {/* Panneau de contrôle central */}
          <div className="absolute top-2 left-1/2 transform -translate-x-1/2 w-10 h-12 bg-gray-900 rounded-lg border border-gray-600">
            {/* Écran principal */}
            <div className="w-full h-8 bg-black rounded-t-lg p-1">
          ...
          <div className="absolute left-0 top-4 -ml-2">
            <div className="w-4 h-2 bg-gray-700 rounded-full transform rotate-12 origin-right">
              <div className="w-2 h-2 bg-blue-500 rounded-full ml-2 animate-pulse"></div>
            </div>
          </div>
          <div className="absolute right-0 top-4 -mr-2">
            <div className="w-4 h-2 bg-gray-700 rounded-full transform -rotate-12 origin-left">
              <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div>
            </div>
          </div>

          {/* Capteurs latéraux */}
          <div className="absolute left-0 top-8 w-2 h-1 bg-yellow-400 rounded-r-full animate-pulse"></div>
          <div className="absolute right-0 top-8 w-2 h-1 bg-yellow-400 rounded-l-full animate-pulse"></div>

          {/* Base avec chenilles */}
          <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-12 h-4 bg-gray-800 rounded-full border-2 border-gray-600">
            <div className="flex justify-between items-center h-full px-1">
              <div className="w-2 h-2 bg-gray-400 rounded-full animate-spin"></div>
              <div
                className="w-2 h-2 bg-gray-400 rounded-full animate-spin"
                style={{ animationDelay: "0.5s" }}
              ></div>
              <div className="w-2 h-2 bg-gray-400 rounded-full animate-spin" style={{ animationDelay: "1s" }}></div>
            </div>
          </div>

          {/* Rayon de scan */}
          {scanMode && (
            <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-32 h-1 bg-gradient-to-r from-transparent via-cyan-400 to-transparent opacity-70 animate-pulse"></div>
          )}
        </div>
        ...
        <div
          ...
          } backdrop-blur-sm text-white px-3 py-2 rounded-lg text-xs font-mono border ${
            isDark ? "border-gray-600" : "border-gray-600"
          } opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none`}
        >
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full bg-gradient-to-r ${getStatusColor()} animate-pulse`}></div>
